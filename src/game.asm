...
load_palettes:
  ...
  ; LDA current_stage
  ; CMP #1
  ; BEQ load_nametable_1
  ; CMP #2
  ; BEQ load_nametable_2
...
collisionMap:
  ...
  CLC
  ADC temp
  TAX     ; use the result as the index

  ; Load the corresponding cell from the collisionMap.
  LDA collisionMap, X

  ; Check if the cell is occupied.
  AND #%00000001  ; check the least significant bit
  BEQ no_collision
  LDA #1
  RTS
...
.proc update_player
  ...
  JSR checkCollition
  BEQ move_left
  INC player_x  ; Undo the decrement if there's a collision.
  JMP check_right
...
check_right:
  ...
  JSR checkCollition
  BEQ move_right
  DEC player_x  ; Undo the increment if there's a collision.
  JMP check_up
...
check_up:
  ...
  ; Check for collision before moving up.
  DEC player_y
  JSR checkCollition
  BEQ move_up
  INC player_y  ; Undo the decrement if there's a collision.
  JMP check_down
...
check_down:
  ...
  JSR checkCollition
  BEQ move_down
  DEC player_y  ; Undo the increment if there's a collision.
  JMP done_checking
